...
const BluetoothScreen: React.FC = () => {
    ...
    useEffect(() => {
        ...
        // Listener for real-time value updates (Verbose Debugging)
        const updateValueListener = BleService.addListener('BleManagerDidUpdateValueForCharacteristic', (data: any) => {
            ...
            // Catching all notification types to find the click event
            if (
                charUuid === '02f00000-0000-0000-0000-00000000ff02' ||
                charUuid === 'd002' ||
                charUuid === 'd004'
            ) {
                if (!data.value) return;
                console.log(`[Tasbeeh] Update on ${charUuid}: [${data.value.join(', ')}]`);

                // Always try to extract count from Byte 1 and 2 if available
                if (data.value.length >= 3) {
                    const cmdEcho = data.value[1];
                    const count = data.value[2] + (data.value[3] ? (data.value[3] << 8) : 0);

                    // Only update if it looks like a valid count change
                    if (count >= 0) {
                        console.log(`[Tasbeeh] Extracted Count: ${count}`);
                        setTasbihCount(count);
                    }

                    // Try to extract battery if available at Byte 4
    ...
    */
    const handleConnect = useCallback(async (device: BleDevice) => {
        ...
        try {
            ...
            // 5. NOTIFICATION (Broad spectrum setup)
    ...
    /**
     * Direct read attempt for ff02
     */
    const directRead = async () => {
        if (!connectedDevice) return;
        const SERVICE_UUID = '02f00000-0000-0000-0000-00000000fe00';
        const ff02 = '02f00000-0000-0000-0000-00000000ff02';
        console.log('[BluetoothScreen] Attempting direct read of ff02...');
        try {
            const data = await BleService.read(connectedDevice.id, SERVICE_UUID, ff02);
            console.log(`[Tasbeeh] Direct Read Response: [${data.join(', ')}]`);
            if (data.length >= 3) {
                const count = (data[1] << 8) + data[2];
                console.log(`[Tasbeeh] Count from direct read: ${count}`);
                setTasbihCount(count);
            }
